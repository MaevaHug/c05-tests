...
cc -Wall -Wextra -Werror -o ft_is_prime main.c ft_is_prime.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_is_prime 2147483647"
...
# Test with nb == 1, expected output is 0
run_test "1" "0" || all_tests_passed=false
# Test with nb == 2, expected output is 1
run_test "2" "1" || all_tests_passed=false
# Test with nb == 9, expected output is 0
run_test "9" "0" || all_tests_passed=false
# Test with nb == INT_MAX, expected output is "2147483647 is a prime number."
run_test "2147483647" "1" || all_tests_passed=false
