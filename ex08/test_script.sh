...
cc -Wall -Wextra -Werror -o ft_ten_queens_puzzle main.c ft_ten_queens_puzzle.c
if [ $? -ne 0 ]; then
...
longest_test="$ ./ft_ten_queens_puzzle > temp_output.txt"
...
run_test() {
	...
	echo "$separator"
	echo "$ ./ft_ten_queens_puzzle > temp_output.txt"

	# Redirect output to a temporary file
	./ft_ten_queens_puzzle > temp_output.txt
	# Extract the line containing the number of solutions
	output=$(grep "There are" temp_output.txt)
	#cat -e temp_output.txt
	rm temp_output.txt
...
# Test cases
# Expected output is the number of solutions
expected="There are 724 solutions"
